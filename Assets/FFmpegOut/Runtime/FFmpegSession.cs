...
namespace FFmpegOut
{
    public sealed class FFmpegSession : System.IDisposable
    {
        ...
        )
        {
            ...
            return CreateWithOutputPath(path, width, height, frameRate, preset, bitrate);
        }
        ...
        )
        {
            return new FFmpegSession(
                ...
                "-y -f rawvideo -vcodec rawvideo -pixel_format rgba -skip_alpha 0"
                + " -colorspace bt709"
                + " -video_size " + width + "x" + height
                + " -framerate " + frameRate 
                + " -loglevel warning -i - " + preset.GetOptions()
                + $" -b:v {bitrate}K -ac 0 -pixel_format yuva420p"
                +" -auto-alt-ref 0 "
                + " \"" + outputPath + "\""
            );
        }
